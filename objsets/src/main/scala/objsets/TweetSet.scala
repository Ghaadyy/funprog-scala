...
abstract class TweetSet extends TweetSetInterface:
  ...
  /**
    * @return `true` if the `TweetSet` is empty.
    */
  def isEmpty: Boolean
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    if p(elem) then left.filterAcc(p, acc.incl(elem)).union(right.filterAcc(p, acc.incl(elem)))
  ...
  def mostRetweeted: Tweet =
    ...
    def maxTweet(left: Tweet, right: Tweet): Tweet = if left.retweets > right.retweets then left else right

    if left.isEmpty && right.isEmpty then return elem

    if left.isEmpty && !right.isEmpty then return maxTweet(elem, right.mostRetweeted)
    if right.isEmpty && !left.isEmpty then return maxTweet(elem, left.mostRetweeted)

    // for any other node
    val leftMostRetweeted: Tweet = left.mostRetweeted
    val rightMostRetweeted: Tweet = right.mostRetweeted
    
    if elem.retweets > maxTweet(leftMostRetweeted, rightMostRetweeted).retweets then elem
    else if leftMostRetweeted.retweets == maxTweet(leftMostRetweeted, rightMostRetweeted).retweets then leftMostRetweeted
